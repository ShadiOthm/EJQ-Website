    <div class="content-box tab-pane fade in active" id="home_owner_report_begin" role="tabpanel">
        <h1><?php echo __('Please inform the date the job begun:'); ?></h1>
<?php
$this->request->data['Job']['date_begin_home_owner'] = "";
echo $this->Form->create('Tender', array('url'=>array('controller'=>'jobs', 'action' => 'home_owner_report_begin')));
echo $this->Form->input('Job.id');
echo $this->Form->input('Tender.id');

echo $this->Form->input("Job.date_begin_home_owner", array(
    'type' => 'text',
